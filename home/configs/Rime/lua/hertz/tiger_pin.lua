...
function pin_processor.init(env)
    env.pin_enable = env.engine.schema.config:get_bool("tiger/pin/enable") or false
...
function pin_processor.func(key_event, env)
    ...
    if key_event.keycode == 0x74 then
        ...
        if Module.str_is_chinese(text) then
...
function pin_filter.init(env)
    ...
    env.indicator = env.engine.schema.config:get_string("tiger/pin/indicator") or "📌"
    user_db.acquire()
end
...
function panacea_translator.init(env)
    ...
    env.infix = env.engine.schema.config:get_string("tiger/pin/panacea/infix") or '//'
    env.escaped_infix = string.gsub(env.infix, "([%^%$%(%)%%%.%[%]%*%+%-%?])", "%%%1")
    env.prompt = env.engine.schema.config:get_string("tiger/pin/panacea/prompt") or "〔加词〕"
    env.indicator = env.engine.schema.config:get_string("tiger/pin/indicator") or "📌"
    env.freestyle = env.engine.schema.config:get_bool("tiger/pin/panacea/freestyle") or false
